...
forecasts_data_target = power_PV_Jacumba[['GB_mean']]
...
# one year (2020)
d_solar_MW = forecasts_data_target['2020-01-01':'2020-12-30']['GB_mean']
...
# 3.8~3.14     day 68 : day 74 
# day 358: day day 364
# 11.5 ~ 11.8   day 310 : day 313
week_PV = source_P_PV[1,309*24+1:313*24+1].T    # positive
week_BESS_discharge = battery_P_d[1,309*24+1:313*24+1].T   # positive
week_BESS_charge = -battery_P_c[1,309*24+1:313*24+1].T   # negative
week_node_input = P_10_21_in[309*24:313*24]    # positive
week_node_output = -P_21_22_out[309*24:313*24]   # negative
week_load = -electric_load['2020-11-05':'2020-11-09']['pu_load'].values*17.5



year_PV = source_P_PV[1,1:].T    # positive
year_BESS_discharge = battery_P_d[1,1:].T   # positive
year_BESS_charge = -battery_P_c[1,1:].T   # negative
year_node_input = P_10_21_in[:]    # positive
year_node_output = -P_21_22_out[:]   # negative
year_load = -electric_load['pu_load'].values*17.5




# load actual BESS charging power and discharging power
actual_BESS = pd.read_csv("C:/WWT/论文/黄老师特刊/supplement material/data/real_bess.csv")
total_actual_charging = actual_BESS[['P_charge_0','P_charge_1']].sum(axis=1)
total_actual_discharging = actual_BESS[['P_discharge_0','P_discharge_1']].sum(axis=1)
...
# actal netload
actual_netload = total_load_30 - actual_solar + total_actual_charging - total_actual_discharging
...
quantile_uf_80 = under_forecast_average_abs.quantile(0.85)



# find error>0
over_forecast = error.where(error > 0)
over_forecast_all = over_forecast.error.sum()

cost_curtailment = 20*over_forecast_all/1000
...
# average_reserve_cost = 25.64
average_reserve_cost = 79.95
uf_cost = average_reserve_cost*quantile_uf_80*8760/1000
